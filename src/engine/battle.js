...
function getAb() {
  ...
  if (typeof module !== 'undefined' && module.exports) {
    _ab = require('./abilities.js');
  } else {
    _ab = window.PileupAbilities;
  }
  return _ab;
}
 ...
 * Uses effectiveLeft / effectiveRight (ability-adjusted values).
 */
 ...
 * each flip's outcome (with effective values and ability events), and final score.
 *
 * Each flip in result.flips has:
 *   index, left, right         — cards (raw)
 *   leftEffective, rightEffective — values used to resolve the flip
 *   winner, delta              — outcome
 *   events                     — array of ability trigger descriptions
 */
function simulateBattle(leftPile, rightPile, selectionFn) {
  ...
  // Pending bonuses carry from one flip to the next via ability triggers.
  var leftPending = 0;
  var rightPending = 0;
  ...
  for (var i = 0; i < flipCount; i++) {
    ...
    // On Reveal bonuses (Blaze: +2 to this card for this flip).
    ...
    // Post-flip ability triggers (On Win / On Loss).
    ...
    if (deltas.leftDelta !== 0) {
      events.push({ side: 'left', ability: leftCard.ability || rightCard.ability, trigger: flip.winner === 'left' ? 'on_win' : 'on_loss', delta: deltas.leftDelta, next: true });
    }
    if (deltas.rightDelta !== 0) {
      events.push({ side: 'right', ability: rightCard.ability || leftCard.ability, trigger: flip.winner === 'right' ? 'on_win' : 'on_loss', delta: deltas.rightDelta, next: true });
    }
